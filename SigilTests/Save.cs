...
namespace SigilTests
{
    ...
    public partial class Save
    {
        ...
        public void EmitAdd()
        {
            ...
            Emit<Func<int, int>>.DefineAssembly(new AssemblyName("Foo"));
            Emit<Func<int, int>>.DefineModule("Bar", filename);
            var myClass = Emit<Func<int, int>>.DefineType("MyClass");

            var e1 = Emit<Func<int, int>>.BuildInstanceMethod(myClass, "E1", MethodAttributes.Public);
            e1.LoadArgument(1);
            e1.LoadConstant(2);
            e1.Add();
            e1.Convert<int>();
